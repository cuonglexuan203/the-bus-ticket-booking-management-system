...
namespace BusTicketManagementApplication.src.layers.businessLayers
{
    ...
    {
        public DataRowView GetUser(string passengerId)
        {
            BusManagementEntities db = new BusManagementEntities();
            return db.V_USERINFOR.Where(d => d["id_passenger"].ToString().Trim() == passengerId).FirstOrDefault();
        }
        public DataRowView GetEmployee(string systemId)
        {
            return new BusManagementEntities().V_EMPLOYEEINFOR.Where(d => d["Employees_ID"].ToString().Trim() == systemId).FirstOrDefault();
        }
        public bool IsAdmin(string employeeId)
        {
            BusManagementEntities db = new BusManagementEntities(StaticEnv.GetDefaultConnectionString());
            return db.V_EMPLOYEEINFOR.Count(d => d["Employees_ID"].ToString().Trim() == employeeId && d["Position"].ToString().Trim() == "administrator") > 0;
        }
        public bool ValidateUser(string username, string password, ref string passengerId, ref string employeeId, ref string errMsg)
        {
            try
            {
                ...
                passengerId = null;
                employeeId = null;
                var curPassenger = db.PASSENGERACCOUNTs.Where(d => d["username"].ToString().Trim() == username && d["password"].ToString().Trim() == password);
                if (curPassenger != null && curPassenger.Count() > 0)
                {
                    passengerId = curPassenger.FirstOrDefault()["id_passenger"].ToString().Trim();
                    return true;
                }
                //
                var curEmployee = db.SYSTEMACCOUNTs.Where(d => d["username"].ToString().Trim() == username && d["pass"].ToString().Trim() == password);
                if (curEmployee != null && curEmployee.Count() > 0)
                {
                    employeeId = db.EMPLOYEEs.Where(d => d["id_account"].ToString().Trim() == curEmployee.FirstOrDefault()["id_account"].ToString().Trim()).FirstOrDefault()["id_employee"].ToString().Trim();
                    return true;
                }
                //
        ...
        public bool CreateNewUser(string username, string password, string name, string phone, ref string passengerId, ref string errMsg)
        {
            ...
            try
            {
                ...
                db.RunNonQueryProc($"pro_CheckUniqueUser '{username}'"); // if there are exist user , throw an sql exception
                //
                // in case of unique username
                string funcName = "func_auto_id_passenger";
                passengerId = db.RunFunc(funcName, new List<string> { });
